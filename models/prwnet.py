#Credit: https://github.com/huofushuo/PRWNet.git
...
def MS(x, beta, gamma):
    '''In the decoder, one can call MS(x, mean, std) with the mean and std form a PONO in the encoder'''
    return x * gamma + beta
...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, dilation=1):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, dilation=1)
...
class DWT_transform(nn.Module):
    def __init__(self, in_channels):
        ...
        self.mean_conv2 = ConvLayer(16, 16, 3, 1, 1)
        self.mean_conv3 = ConvLayer(16, 1, 1, 1)
        
        self.std_conv1 = ConvLayer(1, 16, 1, 1)
        self.std_conv2 = ConvLayer(16, 16, 3, 1, 1)
...
class PALayer_h(nn.Module): #TODO: what's the difference to PALayer_l?
    def __init__(self, channel):
...
class CALayer_low(nn.Module):
    ...
    def forward(self, x):
        # feature descriptor with the global spatial information
        y = self.avg_pool(x)
...
class CALayer_high(nn.Module): #TODO: what's the difference with CALayer_low?
    def __init__(self, channel=1, k_size=3):
...
class ResidualBlock_nbn(nn.Module):
    '''ResidualBlock wihout BatchNormLayer'''
    def __init__(self, channels):
...
class Waveletnet0(nn.Module):
    def __init__(self):
        ...
        self.num = 1
        channel = 16
        self.pre = ConvLayer(3, channel, 11, 1)
        self.conv1 = ResidualBlock(channel)
        self.down1 = ConvLayer(channel, 2*channel, 3, 2)
        self.conv2 = ResidualBlock(2*channel)
        self.down2 = ConvLayer(2*channel, 4*channel, 3, 2)
        self.conv3 = ResidualBlock(4*channel)
...
class Waveletnet0_de(nn.Module):
    def __init__(self):
        ...
        channel = 16
        self.resu_bot = ResidualBlock(4 * channel)
        
        self.dwt3 = DWT_transform(4 * channel)
        self.up_dwt3 = nn.ConvTranspose2d(4*channel, 4*channel, kernel_size=4, stride=2, padding=1)
        self.resud3 = ResidualBlock(4 * channel)
        self.up2 = nn.ConvTranspose2d(4*channel, 2*channel, kernel_size=4, stride=2, padding=1)
        
        self.rp2 = Repoint(4*channel, 2*channel)
        self.resud3_rp2 = ResidualBlock_nbn(2*channel)
        
        self.dwt2 = DWT_transform(2*channel)
        self.up_dwt2 = nn.ConvTranspose2d(2*channel, 2*channel, kernel_size=4, stride=2, padding=1)
        
        self.resud2 = ResidualBlock(2*channel)
        self.up1 = nn.ConvTranspose2d(2*channel, channel, kernel_size=4, stride=2, padding=1)
        
        self.rp1 = Repoint(2*channel, channel)
        self.resud2_rp1 = ResidualBlock_nbn(channel)
        
        self.dwt1 = DWT_transform(channel)
        self.up_dwt1 = nn.ConvTranspose2d(channel, channel, kernel_size=4, stride=2, padding=1)
        
        self.resud1 = ResidualBlock(channel)
        self.convd0 = ConvLayer(channel, 3, 3, 1)
